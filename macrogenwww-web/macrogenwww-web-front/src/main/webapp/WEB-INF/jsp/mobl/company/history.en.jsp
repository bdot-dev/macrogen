...
<body class="en">
    ...
    <div class="full-bg" style="background-image: url(/publishing/mobile-en/dist/img/macrogen/macrogen-history-bg.png);">
                ...
                <div class="swiper-wrapper">
                   	<c:forEach var="result" items="${ resultList }" varStatus="status">
	                    <div class="swiper-slide ${ status.index eq 0 ? 'on' : '' }"><a
	                    	href="javascript:;"><span class="text">${result.beginYear } - ${result.endYear eq 'present' ? 'Present' : result.endYear }</span></a></div>
	                </c:forEach>
                </div>
	               	...
	               	<c:if test="${fn:length(result.yearCmpnyhistList) gt 0 }">
		                <div class="section ${ status.index eq 0 ? 'show' : '' } _section">
		                    ...
		                    <div class="history-heading" data-aos="fade-up" data-aos-duration="2000">
		                        <div class="year">${result.beginYear } - ${result.endYear eq 'present' ? 'Present' : result.endYear }</div>
	         ...
	         $(window).on('scroll',function(){
	             ...
	             $('.section.show .history-list').each(function(idx) {
	                 if(offset > $('.section.show .history-list').eq(idx).offset().top - 62) {
	                     ...
	                     let text = $(this).children('.year').text();
	                     $(".section.show .select-box a").css({
	                         'color' : '#454545',
	                         'font-weight' : 'normal'
	                     });
	                     $(".section.show .select-box a:contains(" + text + ")").css({
	                         'color' : '#2E87EF',
	                         'font-weight' : 'bold'
	                     });
	                 }
