...
<body>
		...
		<div class="search_wrap">
			<fieldset>
				<div class="list_wrap">
					...
					<select v-model="listVo.searchCntntsCtgryCode" name="searchCntntsCtgryCode" style="width:200px;">
						<option :value="null">전체</option>
						<option v-for="(result, index) in cntntsCtgryCodeList"
					...
					<tr v-for="(result, index) in resultList">
						<td>
							<input type="checkbox" v-model="checkedPkList"
								...
								:value="result.cntntsSn">
						</td>
						<td>{{paginationInfo.totalRecordCount + 1 - ((paginationInfo.currentPageNo-1) * paginationInfo.recordCountPerPage + (index + 1)) }}</td>
						<td>
							<img :src="'${publicUrl }' + result.imageFlpth"
								style="width:100px; vertical-align: bottom;" />
						</td>
						<td>{{ result.cntntsCtgryCodeNm }}</td>
						<td class="tal"><a v-on:click="onViewLink(result.cntntsSn)"
							href="javascript:;">{{ result.cntntsSj }}</a></td>
	...
	<script src="/js/app/SocialContentsList.js"></script>
	<script>
	$(function () {
		var listVo = {
			pageIndex : ${ listVo.pageIndex },
			recordCountPerPage : 10,
			searchCntntsCtgryCode: ${ empty listVo.searchCntntsCtgryCode ? 'null' : "'".concat(listVo.searchCntntsCtgryCode).concat("'") },
