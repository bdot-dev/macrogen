			...
			methods : {
				fetchData : function() {
					...
					$.ajax({ dataType : 'json', type : 'post',
						...
						url : '/' + options.lang + '/wnpz/list/data',
				...
				onAdd : function() {
					$form.attr({
						action : '/' + options.lang + '/wnpz/form',
						method : 'post'
					}).submit();
				},
				onViewLink : function(sn) {
					$form.attr({
						action : '/' + options.lang + '/wnpz/form/' + sn,
						method : 'post'
					}).submit();
				},
				...
				checkAll: function(e) {
					...
					if (e.target.checked) {
						vm.resultList.forEach(function(result) {
							vm.checkedPkList.push(result.wnpzSn);
						});
					}
				},
				onDeleteChecked: function() {
					...
					$.ajax({dataType : 'json', type : 'post',
						contentType : 'application/json',
						url : '/' + options.lang + '/wnpz/deleteList',
						data : JSON.stringify({ wnpzSnList : vm.checkedPkList }),
					}).done(function(data) {
						vm.fetchData();
						vm.checkedPkList = [];
					});
