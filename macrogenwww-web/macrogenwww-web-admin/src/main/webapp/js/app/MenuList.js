var MenuList = (function($) {
	...
	var onSelectMenuNode = function(e, treedata) {
		$.ajax({type: 'post', dataType: 'json',
			...
			success: function(data) {
				if (data && data.result === 'success') {
					menuVue.menu = data.menu;
					menuVue.expsrYnList = data.expsrYnList;
					menuVue.menuSeCodeList = data.menuSeCodeList;

					$.cookie("menu.selectedNode.id", treedata.node.id);
				} else {
					alert('데이터 처리 도중 에러가 발생하였습니다.');
					return false;
				}
	...
	var onCreateMenuNode = function(e, treedata) {
		...
		$.ajax({type: 'post', dataType: 'json',
			...
			success: function(data) {
				if (data && data.result === 'success') {
					$jstree.jstree(true).set_id(treedata.node, data.menu.id);
					$jstree.jstree(true).set_text(treedata.node, data.menu.text);
				} else {
					alert('데이터 처리 도중 에러가 발생하였습니다.');
	...
	var onloadedMenuNode = function() {
		...
		if ($.cookie("menu.selectedNode.id")) {
			var nodeId = $.cookie("menu.selectedNode.id");
