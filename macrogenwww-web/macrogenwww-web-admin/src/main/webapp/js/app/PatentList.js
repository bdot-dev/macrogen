			...
			methods : {
				fetchData : function() {
					...
					$.ajax({ dataType : 'json', type : 'post',
						...
						url : '/' + options.lang + '/patent/list/data',
				...
				onAdd : function() {
					$form.attr({
						action : '/' + options.lang + '/patent/form',
						method : 'post'
					}).submit();
				},
				onViewLink : function(sn) {
					$form.attr({
						action : '/' + options.lang + '/patent/form/' + sn,
						method : 'post'
					}).submit();
				},
				...
				checkAll: function(e) {
					...
					if (e.target.checked) {
						vm.resultList.forEach(function(result) {
							vm.checkedPkList.push(result.patentSn);
						});
					}
				},
				onDeleteChecked: function() {
					...
					$.ajax({dataType : 'json', type : 'post',
						contentType : 'application/json',
						url : '/' + options.lang + '/patent/deleteList',
