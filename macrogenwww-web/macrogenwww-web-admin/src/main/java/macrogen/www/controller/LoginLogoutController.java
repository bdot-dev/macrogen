	...
	public ModelAndView loginForm(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		if(ipChk) {
			...
			if ("XMLHttpRequest".equals(request.getHeader("x-requested-with"))) {	//Ajax 요청이면
				...
				if (StringUtils.hasText(CookieUtil.getCookie(request, "mngr_id"))) {	//쿠키에서 뽑아온 아이디 값 에 즉 문자열에 공백이 들어가 있는지 검사
					
					try {
						
						model.addAttribute("savedMngrId", aesCipher.decrypt(CookieUtil.getCookie(request, "mngr_id")) );	//AES-128 복호화
			...
			LOGGER.info("accessError : LoginLogoutController");
			mav.setViewName("redirect:/accessError.jsp");
		}
